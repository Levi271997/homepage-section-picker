 ...
 * miming it — and the sign follows the open state through CSS rather than a
 * prop. The browser's own disclosure triangle is turned off; the plus is the
 * design's marker.
 */
function QuestionRow({
  ...
  return (
    <li>
      ...
      <details open={open} data-role="row" className={`group ${shell}`}>
        <summary className="flex cursor-pointer list-none items-start justify-between gap-[2cqw] [&::-webkit-details-marker]:hidden">
          {question ? (
            <span className="text-[1.7cqw] leading-tight font-medium text-neutral-900">{question}</span>
          ) : (
            <span aria-hidden="true" className="h-[1.6cqw] w-[52%] rounded-full bg-neutral-900" />
          )}
          <span aria-hidden="true" className="shrink-0 text-[3cqw] leading-none font-medium" style={{ color: GREEN }}>
            <span className="group-open:hidden">+</span>
            <span className="hidden group-open:inline">−</span>
          </span>
        </summary>

        {answer && <p className="mt-[1.4cqw] text-[1.4cqw] leading-[1.45] text-neutral-600">{answer}</p>}
      </details>
    </li>
  )
}
...
export default function FaqPreview({
  ...
  if (layout === 'split') {
    ...
    return (
      <section aria-label="Frequently asked questions" className="flex h-full items-start gap-[5cqw] px-[5cqw] py-[4cqw]">
  ...
  return (
    <section aria-label="Frequently asked questions" className="flex h-full flex-col gap-[3.5cqw] px-[5cqw] py-[4cqw]">
