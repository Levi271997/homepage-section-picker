...
export const CONTENT: Record<string, Schema> = {
  ...
  'contact-form': {
    fields: [
      ...
      { id: 'fieldLabels', label: 'Form fields', kind: 'lines', hint: 'One label per line. The last one is the message box.' },
      { id: 'fieldHints', label: 'Field examples', kind: 'lines', hint: 'The greyed example inside each field, matching the labels above.' },
      { id: 'consent', label: 'Consent line', kind: 'paragraph' },
      cta('Submit button'),
      { id: 'cta2', label: 'Copy button', kind: 'text', hint: 'Beside the pitch, on the four panel designs. V5 and V6 draw no pair.' },
      { id: 'cta3', label: 'Copy button, secondary', kind: 'text' },
      image(),
    ],
    defaults: {
      eyebrow: 'Contact',
  ...
  pricing: {
    fields: [
      ...
      heading(),
      body(),
      { id: 'headerCta', label: 'Header button', kind: 'text', hint: 'The button under the heading. The plan button is set below.' },
