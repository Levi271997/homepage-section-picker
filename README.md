...
- Options are grouped into independent **axes** (`OptionGroup`). A group renders either as preview cards (`display: 'cards'`) or a compact chip row (`display: 'chips'`), and every preview card renders its option combined with the section's *other* current choices. A group can declare `appliesTo(choice)` so axes the current layout doesn't use are hidden from both the picker and the caption — e.g. "Image on" disappears when the content section isn't a split layout. Sections configured so far:
  ...
  | Call to action | Design: the 10-strong V1…V10 set — see [Design sets](#design-sets) |
  | Pricing | Design: the 6-strong V1…V6 set — see [Design sets](#design-sets) |
  | Blogs | Design: the 6-strong V1…V6 set — see [Design sets](#design-sets) |
  | Contact form | Design: the 6-strong V1…V6 set — see [Design sets](#design-sets) |
  | Site footer | Design: the 8-strong V1…V8 set — see [Design sets](#design-sets) |
...
- **Logo strip** — three designs (V1…V3). Figma pluralised this one: the folder and the files both read "Logo Strips". V1 and V2 page their row on [Swiper](https://swiperjs.com), a slide to a logo so the strip moves one at a time with six in view, and it runs itself in the full-page view — three seconds a logo, paused on hover and off entirely under `prefers-reduced-motion`. V3 is three rows standing still.
...
- **Site footer** — eight designs (V1…V8), exported under `Style=` rather than `Type=` like the content section. Three frames: the tall footer with link columns, a strip built round the logo, and a bare line of type.
- **Contact form** — six designs (V1…V6). Figma pluralised this one and the folder didn't follow: `section-cogs/contact form/Type=Contact Forms V1.svg`. All six are the same five-field form; what varies is what holds it and which side it sits on.
